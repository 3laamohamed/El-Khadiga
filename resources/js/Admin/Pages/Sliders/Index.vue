    ...
    <div class="card mb-5 mb-xl-10">
        <div class="card-header border-0 cursor-pointer">
            <div class="card-title m-0">
                <h3 class="fw-bolder m-0">سلايدرز</h3>
            </div>
            <div class="card-toolbar">
                ...
                <Link class="nav-link btn btn-sm  me-2"  style="background-color: #115da6 ;color: #ffffff" href="/admin/sliders/create">إضافة</Link>
            </div>
        </div>
                ...
                <table class="table table-hover table-rounded table-row-bordered border gy-3 gs-3">
                    ...
                    <tr v-for="(item,index) in services.data" :key="item.id" :class="{'table-active': index % 2 !== 0 }">
                        ...
                        <td class="text-center border">
                            <Link :href="`/admin/sliders/${item.id}/edit`"  >
...
let delete_shop = (id) => {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            form.delete('/admin/sliders/'+ id , { preserveState: true,
                replace: true,
                preserveScroll: true,})
        }
    });
